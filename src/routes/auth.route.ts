...
authRoutes.post('/login', async (c) => {
  try {
    ...
    const identifier = asTrimmedString(
      body.identifier ?? body.email ?? body.phone ?? body.username
    );
...
authRoutes.post('/admin/request-password-setup', async (c) => {
  try {
    ...
    const isForbidden =
      message.includes('Admin account required') ||
...
authRoutes.post('/admin/create-password', async (c) => {
  try {
    ...
    if (!identifier || !verificationCode || !newPassword) {
      return c.json(
        { error: 'identifier, verificationCode, and newPassword are required' },
        400
      );
    }
