...
if (!supabaseUrl || !supabaseAnonKey) {
    console.warn('Supabase auth is not fully configured. Set SUPABASE_URL and SUPABASE_ANON_KEY.');
}
...
export const verifySupabaseAccessToken = async (accessToken) => {
    if (!supabaseAuthClient || !accessToken)
        return null;
...
export const signInSupabaseWithEmailPassword = async (email, password) => {
...
export const verifySupabaseEmailOtp = async (email, token) => {
    if (!supabaseAuthClient) {
        throw new Error('Supabase auth client is not configured');
    }
