...
const asQueueNumberString = (value) => {
    if (typeof value === 'number' && Number.isFinite(value)) {
...
const normalizeEnvelope = (raw) => {
    if (!raw || typeof raw !== 'object') {
        return { ok: false, body: { error: 'Request body must be a JSON object' } };
    }
    const body = raw;
...
const diffFields = (payload, serverRecord) => {
    return Object.keys(payload).filter((key) => {
        if (key === 'version' || key === 'source_device_id')
            return false;
...
const hashRequest = (envelope) => {
    return createHash('sha256').update(JSON.stringify(envelope)).digest('hex');
};
const toSyncStatus = (value) => {
    if (value === 400 || value === 409 || value === 503)
        return value;
    return 200;
};
const idempotencyLookup = async (key, entityType, operation) => {
...
const idempotencyStore = async (envelope, requestHash, status, payload) => {
    ...
    `, envelope.idempotencyKey, envelope.entityType, envelope.operation, envelope.entityId, requestHash, status, JSON.stringify(payload));
};
const withIdempotency = async (envelope, run) => {
    const requestHash = hashRequest(envelope);
    const replay = await idempotencyLookup(envelope.idempotencyKey, envelope.entityType, envelope.operation);
    if (replay) {
...
const readUnloadingById = async (id) => {
    const rows = (await prisma.$queryRawUnsafe(`select * from public.sync_unloading_record where id = $1 limit 1`, id));
    return rows[0] ?? null;
};
const readTicketById = async (id) => {
    const rows = (await prisma.$queryRawUnsafe(`select * from public.sync_ticket_record where id = $1 limit 1`, id));
    return rows[0] ?? null;
};
const buildConflict = (entityType, entityId, message, serverRecord, payload) => {
...
const requirePayloadFields = (payload, fields) => {
...
export const upsertTicketFromSync = async (raw) => {
    const normalized = normalizeEnvelope(raw);
    if (!normalized.ok)
        return { ok: false, status: 400, body: normalized.body };
    ...
    return withIdempotency(envelope, async () => {
        ...
        if (!required.ok)
            return { ok: false, status: 400, body: required.body };
        const transactionRows = (await prisma.$queryRawUnsafe(`select * from public.sync_ticket_record where transaction_id = $1 limit 1`, asString(envelope.payload.transaction_id)));
        if (transactionRows[0] && transactionRows[0].id !== envelope.entityId) {
            return buildConflict('tickets', envelope.entityId, 'transaction_id already exists on another record', transactionRows[0], envelope.payload);
        }
        ...
        if (!existing) {
          ...
          values
            ($1, $2, $3, $4, $5, $6, $7::timestamptz, $8::timestamptz, $9::numeric, $10::timestamptz,
             $11::jsonb, $12, $13)
          returning *
        `, envelope.entityId, asString(envelope.payload.transaction_id), asString(envelope.payload.ticket_no), asString(envelope.payload.vehicle_number), asString(envelope.payload.vehicle_type), asString(envelope.payload.goods_type), asString(envelope.payload.unloading_time), asString(envelope.payload.suggested_unloading_time), asNumber(envelope.payload.total_amount) ?? 0, asString(envelope.payload.issued_at), JSON.stringify(envelope.payload.items), envelope.sourceDeviceId ?? null, envelope.idempotencyKey));
...
const upsertUnloading = async (raw, markDone) => {
    const normalized = normalizeEnvelope(raw);
    if (!normalized.ok)
        return { ok: false, status: 400, body: normalized.body };
    const envelope = normalized.data;
    return withIdempotency(envelope, async () => {
        ...
        if (!existing) {
          ...
          returning *
        `, envelope.entityId, normalizedQueueNumber, asString(envelope.payload.vehicle_number), asString(envelope.payload.vehicle_type), asString(envelope.payload.product_category), asString(envelope.payload.started_at), asNumber(envelope.payload.estimated_minutes) ?? 0, markDone ? asString(envelope.payload.completed_at) || new Date().toISOString() : null, envelope.sourceDeviceId ?? null, envelope.idempotencyKey));
            return { ok: true, status: 200, body: { success: true, data: rows[0] } };
        }
        const expectedVersion = asNumber(envelope.payload.version);
        const actualVersion = asNumber(existing.version) ?? 0;
        ...
        if (expectedVersion !== actualVersion) {
            return buildConflict('unloading', envelope.entityId, 'Unloading record version mismatch', existing, envelope.payload);
        }
        const rows = (await prisma.$queryRawUnsafe(`
        ...
        where id = $1
        returning *
      `, envelope.entityId, normalizedQueueNumber, asString(envelope.payload.vehicle_number), asString(envelope.payload.vehicle_type), asString(envelope.payload.product_category), asString(envelope.payload.started_at), asNumber(envelope.payload.estimated_minutes) ?? 0, markDone, asString(envelope.payload.completed_at) || null, envelope.sourceDeviceId ?? null, envelope.idempotencyKey));
        return { ok: true, status: 200, body: { success: true, data: rows[0] } };
    });
};
export const startUnloadingFromSync = async (raw) => upsertUnloading(raw, false);
export const completeUnloadingFromSync = async (raw) => upsertUnloading(raw, true);
export const getUnloadingSnapshot = async () => {
    const rows = (await prisma.$queryRawUnsafe(`select * from public.sync_unloading_record order by updated_at desc, id asc`));
    return { success: true, data: rows };
};
export const getTicketsSnapshot = async () => {
    const rows = (await prisma.$queryRawUnsafe(`select * from public.sync_ticket_record order by updated_at desc, id asc`));
    return { success: true, data: rows };
};
