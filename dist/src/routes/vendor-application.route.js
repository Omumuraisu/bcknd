...
import { approveVendorApplication, listPendingVendorApplicationsByBusiness, requestVendorApplicationCompliance, } from '../services/admin-services/vendor-application.service.js';
const vendorApplicationRoutes = new Hono();
const isDevelopment = process.env.NODE_ENV !== 'production';
vendorApplicationRoutes.use('*', cors({
...
vendorApplicationRoutes.post('/:id/compliance-request', requireAdminCapability('admin:write'), async (c) => {
    try {
        const id = BigInt(c.req.param('id'));
        const body = (await c.req.json());
        const requiredDocuments = body.requiredDocuments;
        const note = asTrimmedString(body.note);
        const application = await requestVendorApplicationCompliance(id, requiredDocuments, note || undefined);
        return c.json({
