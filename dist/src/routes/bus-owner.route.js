...
const normalizeCreateBusinessOwnerBody = (body) => {
    ...
    const contact_number = asTrimmedString(pickFirst(input, ['contact_number', 'contactNumber', 'phone']));
    const email = asTrimmedString(input.email);
    const businessName = asTrimmedString(pickFirst(input, ['businessName', 'business_name']));
    ...
    if (!BUSINESS_TYPES.has(businessTypeRaw)) {
        return {
            ok: false,
            status: 400,
...
userRoutes.post('/request-activation', async (c) => {
    try {
        ...
        const contactNumber = asTrimmedString(pickFirst(body, ['contact_number', 'contactNumber', 'phone']));
        if (!contactNumber) {
